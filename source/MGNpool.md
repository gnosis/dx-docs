...
Users may participate in this pool with their funds, which they will reiceve back at the end of the trading period (or more precisely: the value of their deposit at the end of the pooling period (may have increased or decreased). 
...
- This process continues until the pre-defined time has been reached and an even number of auctions have run
...
- Note that two contracts should be launched so that the opposite auctions always have funding (e.g. WETH-TOKEN and TOKEN-WETH at the same time). 
...
- Note that though this ensures sellSide liquidity, it is highly advised to ensure the bidder side in case not enough liquidity is on the DutchX already (e.g. by bots between the opposite auctions or a bot to another exchange)
